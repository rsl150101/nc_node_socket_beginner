...
const handleWelcomeFormSubmit = (e) => {
  ...
  socket.emit("roomSizeCheck", roomName);
};
...
//- socket
socket.on("roomSize", async (size) => {
  if (size !== 2) {
    await initCall();
    socket.emit("joinRoom", roomName);
  } else {
    const welcomeFull = welcome.querySelector("h3");
    welcomeFull.innerText = `${roomName} is full`;
  }
});
